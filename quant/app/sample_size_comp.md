...
Leave one of the inputs blank to determine its value. By default the sample size input is left empty so sample size is calculated. Only one input can be left blank.

## Input

* Sample size: Number of respondents required
* Confidence level: 1 - significance level (e.g, .95 = 1 - .05). The significance
* Power: 1 - $\beta$ (e.g, .8 = 1 - .2)
* Ratio: Relative sample size for group 1 (control) and group 2 (test)

## Input for a comparison of means

* Delta: Difference between group means to we hope to detect
...
* Proportion 2: Poportion 1 plus the difference we hope to detect (e.g., .1 + .05 = .15)

**Note:** The (matching) ratio of _n1_ (control group sample size) to _n2_ (test group sample size) is 1 by default. If no value is provided a value of 1 is assumed.
