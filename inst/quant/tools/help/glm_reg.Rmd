...
To generate predicted values for all cases in, for example, the `titanic` dataset click the `Data` radio button and then select the dataset from the dropdown. You can also create a dataset for input in Excel and then paste it into Radiant through the Data > Manage menu. You can also load csv data as input. For example, load this [example](https://github.com/mostly-harmless/radiant/blob/master/examples/glm_pred.csv?raw=true) csv file into Radiant through the Data > Manage tab and try to generate the same predictions.
...
![Plot](figures_quant/summary_glm_reg.png)

Probabilities, are more convenient for interpretation than either the coefficients from a logit (probit) model or the odds. To see how survival probabilities change across passenger classes click the `Command` radio button, type `pclass = levels(pclass)` in the **Prediction command** box, and press Enter.

![Plot](figures_quant/summary_glm_reg_pred.png)

For males of average age (approx. 30 in the sample) the survival probability was close to 50%. For 3rd class passengers this probability was closer to 9%.
...
To see the effects of gender type `sex = levels(sex)` in the **Prediction command** box and press Enter. For average age females in 3rd class the survival probability was around 50%. For males with the same age and class characteristics the chance of survival was closer to 9%.
...
To see the effects for age type `age = seq(0,100, 20)` in the **Prediction command** box. For male infants in 3rd class the survival probability was around 20%. For 60 year old males in 3rd class the probability drops to around 3%. For the oldest males on board the model predicts a survival probability close to 0%.
...
For a more comprehensive overview of the influence of gender, age, and passenger class on the chances of survival we can generate a full table of probabilities using the following command: `pclass = levels(pclass), sex = levels(sex), age = seq(0,100, 20)`. There are too many numbers to easily interpret in table form so we save the predictions (you can use the `Prediction` button) and paste the table into Radiant through the Data > Manage tab. Finally, generate the following plot in the Data > Visualize tab:

![Probability](figures_quant/viz_glm_reg_pred.png)
