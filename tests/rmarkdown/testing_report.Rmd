...
result <- compare_means(dataset = "anscombe", var1 = "y1", var2 = c("x2", "y3"), samples = "independent", alternative = "two.sided", conf_level = 0.95, adjust = "none")
summary(result)
plot(result, cm_plots = "bar")
```

```{r fig.width=7, fig.height=4}
result <- regression("anscombe", "y1", "x2")
summary(result)
...
```{r fig.width=7, fig.height=4}
result <- regression("ideal", "y", "x2")
summary(result)
plot(result, reg_plots = "coef")
```

```{r fig.width=6.46, fig.height=8.4}
result <- visualize("diamonds", xvar = "carat", yvar = "price", type = "scatter", color = "clarity", check = c("line", "loess"), axes = c("log_x", "log_y"))
```

```{r}
result <- sampling("rndnames", var = "Names", sample_size = 6)
summary(result)
```
